...
function TaskRow({ task, onToggle, onDelete, onEdit }: { task: Task; onToggle: (id: string) => void; onDelete: (id: string) => void; onEdit?: (t: Task) => void }) {
  ...
  return (
    <div className={`flex items-center gap-2 p-2 rounded-xl hover:bg-white/[0.03] ${task.completed ? 'opacity-40' : ''}`}>
      <button
      ...
      >
        {task.completed && <svg className="w-3 h-3 text-[#050607]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>}
      </button>
      <button onClick={() => setShowActions(!showActions)} className="flex-1 min-w-0 text-left">
